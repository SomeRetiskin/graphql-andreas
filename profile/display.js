...
export async function displayUserName() {
  try {
    ...
    if (userData) {
      ...
      if (userElement && userInfoContainer) {

        userElement.textContent = fullName;

        userElement.addEventListener("mouseenter", async function (event) {
          try {
            const info = await fetchUser();
            if (info) {
              const user = info.login;
              const attrs =
                typeof info.attrs === "string"
                  ? JSON.parse(info.attrs)
                  : info.attrs;
              const tel = attrs.tel;
        ...
        userElement.addEventListener('mousemove', function (event) {
          userInfoContainer.style.top = `${event.clientY + 10}px`;
          userInfoContainer.style.left = `${event.clientX + 10}px`;
        });
  ...
  clickableContainer.addEventListener("click", async function (event) {
    ...
    try {
      ...
      if (info) {
        const user = info.login;
        const attrs =
          typeof info.attrs === "string" ? JSON.parse(info.attrs) : info.attrs;
        const tel = attrs.tel;
        const email = attrs.email;
...
export async function div01Chart() {
  ...
  const divPaths = data.transactions.filter(
    (transaction) =>
  ...
  const modifiedPaths = divPaths.map((transaction) => {
    ...
    const shortName =
      strippedPath.split("/").pop().substring(0, 10) +
      (strippedPath.length > 10 ? "..." : "");
